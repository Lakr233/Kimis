...
class FollowerController: ViewController {
    let userId: String
    ...
    var userList = [FollowRecord]() {
        didSet { tableView.users = userList.compactMap(\.follower) }
    }
    ...
    func loadMore() {
        ...
        DispatchQueue.global().async {
            ...
            let items = source.req.requestForUserFollowers(
                userId: userId,
                untilId: untilId,
            )
...
extension FollowerController: LLNavControllerAttachable {
